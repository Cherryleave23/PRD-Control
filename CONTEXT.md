...
**硬阻断（Hard Block）**：
对违反不变式的偏离，agent 自动返工。**不提示操作者**指的是不弹出确认框打断用户、不询问"是否返工"（决策已由不变式预设）。返工完成后向用户输出简洁汇总（信息同步，非决策请求），并立即写入 `docs/deviations.md`。是"不阻断"原则的唯一例外。
_避免_：强制返工、自动回滚

**强提醒（Strong Reminder）**：
偏离处理的三级光谱中间档。仍不阻断（决定权在用户），但风险可见性更高 + 建议更明确。典型场景：返工缺陷 + 功能异常、模型自添加设计 + 功能异常。与"软提醒"的区别在于输出的降级标识：软提醒输出后继续执行；强提醒输出附带"强烈不建议继续，建议先解决此偏离再推进"，并暂停等待用户确认。仍不阻断（用户可否决）。
_避免_：强制提示、高风险提醒
