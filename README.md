...
| 偏离类型 | 处理方式 |
|---|---|
| **违反不变式**（=与设计边界完全相反） | **硬阻断** —— agent 自动返工，不询问用户；返工完成后输出简洁汇总并立即写入 deviations.md |
| 越界 / 缺斤少两 / 触碰范围外 | **软提醒** / **强提醒** —— 提示用户 + 给建议，继续执行（强提醒暂停等待确认） |

硬阻断是"不阻断"原则的**唯一例外**，边界严格限定为"违反不变式"。理由：不变式是系统在任何情况下都要满足的约束，违反它意味着系统进入未定义状态，可能引发数据损坏或级联故障。"不提示"指不询问"是否返工"（决策已预设），非完全不告知。
...
不变式是硬阻断的触发条件，分类决策直接影响偏离分级。本生态明确**分类决策由 `spec-grilling` 在访谈中做，不由 `spec-modeling` 在写入时做**——避免写入时重做决策导致分类漂移。

### 流程

1. 用户在访谈中提出一条要求
2. `spec-grilling` 主动提议分类（不变式 / 需求 / 边界 / ADR），给推荐 + 理由
3. 用户拍板确认分类
4. `spec-modeling` 按已确定的分类写入 SPEC.md 对应段落（**不重新做分类决策**）

### 精炼三问（判断是否升格为不变式）

`spec-grilling` 提议分类时，用精炼三问自检——三问全过才推荐定为不变式：
...
### 可证伪性自检（spec-modeling 写入前）

`spec-modeling` 写入不变式前做轻量表述质量检查（仅精炼三问第 3 问）：能写出返回 true/false 的检查函数吗？不能则提示用户重写表述，但仍写入（**不阻断**，保留用户控制权）。此自检只兜底表述质量，不改分类。
